...
date = gets.chomp
asteroid_data = NasaApiService.new.get_asteroids(date)
...
column_data = column_labels.each_with_object({}) do |(col, label), hash|
  hash[col] = {
    label: label,
    width: [asteroid_data.map { |asteroid| asteroid[col].size }.max, label.size].max}
end
...
puts "On #{formatted_date}, there were #{Asteroid.total_number_of_asteroids(date)} objects that almost collided with the earth."
puts "The largest of these was #{Asteroid.largest_asteroid_diameter(date)} ft. in diameter."
puts "\nHere is a list of objects with details:"
puts divider
puts header
create_rows(Asteroid.formatted_data(date), column_data)
puts divider
